...
class Example(QtWidgets.QMainWindow, Ui_Form):
    ...
    def new_number(self, old_number, op):
    ...
    def operation(self):
        ...
        if tx in '0123456789':
            val = self.table.value()
            if self.point:
                self.table.display(self.table.value() + float(tx) / (10 ** self.step))
                self.step += 1
            elif val == 0.0:
                self.table.display(tx)
            else:
                self.table.display(self.table.value() * 10 + float(val))

        elif tx == '.':
            if not self.point:
                self.point = True
        elif tx in '+-/*^':
            self.new_number(self.table.value(), tx)
            self.table.display(0)
        elif tx == '√':
            self.table.display(self.table.value() ** 0.5)
        elif tx == '!':
            val = math.factorial(self.table.value())
            self.table.display(val)
        elif tx == '=':
            if self.op == '+':
                self.table.display(self.old_number + self.table.value())
            elif self.op == '-':
                self.table.display(self.old_number - self.table.value())
            elif self.op == '*':
                self.table.display(self.old_number * self.table.value())
